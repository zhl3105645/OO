...
public class JvmHeap extends MyHeap<MyObject> {
    ...
    public void removeUnreferenced() {
        ...
        int num = 0;
        for (int i = 1; i <= this.getSize(); i++) {
            if (((MyObject) this.getElementData()[i]).getReferenced()) {
                this.setElementData(++num, (MyObject) this.getElementData()[i]);
            }
        }
        this.setSize(num);
    }
    ...
    public /*@pure@*/ MyObject getYoungestOne() {
        ...
        if (this.getSize() == 0) {
            ...
            int min = 1;
            for (int i = 1; i <= this.getSize(); i++) {
                if (((MyObject) this.getElementData()[i]).compareTo((MyObject) this.getElementData()[min]) == -1) {
                    min = i;
                }
            }
            return (MyObject) this.getElementData()[min];
        }
    }
}