...
public class MyUmlClassModelInteraction implements UmlClassModelInteraction {
    ...
    private HashMap<String, UmlAssociationEnd> idToAssociationEnd = new HashMap<>();
    private HashMap<String, String> endToReference = new HashMap<>();
    private boolean allNameExist = true;
    ...
    void initial1(UmlElement... elements) {
        for (UmlElement element : elements) {
            ...
            if (element.getElementType() == ElementType.UML_CLASS) {
                ...
                idToAssociationEnd.put(element.getId(), (UmlAssociationEnd) element);
        ...
        for (UmlElement element : elements) {
            if (element.getElementType() == ElementType.UML_ASSOCIATION) {
                ...
                if (idToClass.containsKey(endToReference.get(umlAssociation.getEnd1()))
                    ...
                    class1.addAssociationEnd(idToAssociationEnd.get(umlAssociation.getEnd2()));
                    class2.addAssociation(class1);
                    class2.addAssociationEnd(idToAssociationEnd.get(umlAssociation.getEnd1()));
                }
                ...
                } else if (idToInterface.containsKey(umlAttribute.getParentId())) {
                    idToInterface.get(umlAttribute.getParentId()).addAttribute(umlAttribute);
                }
            }
        }
    }
    ...
    void checkNameIsExist(UmlElement... elements) {
        for (UmlElement element : elements) {
            if (element.getElementType() == ElementType.UML_CLASS
                    || element.getElementType() == ElementType.UML_ATTRIBUTE
                    || element.getElementType() == ElementType.UML_OPERATION
                    || element.getElementType() == ElementType.UML_INTERFACE
                    || (element.getElementType() == ElementType.UML_PARAMETER
                    && ((UmlParameter) element).getDirection() != Direction.RETURN)) {
    ...
    public HashSet<AttributeClassInformation> role1Check() {
        HashSet<AttributeClassInformation> information = new HashSet<>();
        for (String id : idToClass.keySet()) {
            information.addAll(idToClass.get(id).getSameAttributeInformation());
        }
        return information;
    }

    public HashSet<UmlClassOrInterface> getCircularInheritance() {
        HashSet<UmlClassOrInterface> information = new HashSet<>();
        for (String id : idToClass.keySet()) {
            if (!information.contains(idToClass.get(id).getUmlClass())) {
                ArrayList<UmlClass> classes = idToClass.get(id).getCircularInheritance();
                if (classes != null) {
                    information.addAll(classes);
                }
            }
        }
        for (String id : idToInterface.keySet()) {
            if (idToInterface.get(id).isCircle()) {
                information.add(idToInterface.get(id).getUmlInterface());
            }
        }
    ...
    public HashSet<UmlClassOrInterface> getDupGeneralization() {
        HashSet<UmlClassOrInterface> information = new HashSet<>();
        for (MyUmlInterface myUmlInterface : idToInterface.values()) {
            if (myUmlInterface.isDupGeneralization()) {
                information.add(myUmlInterface.getUmlInterface());
            }
        }
        return information;
    }

    public HashSet<UmlClass> getDupInterfaceOfClass() {
        HashSet<UmlClass> information = new HashSet<>();
        for (MyUmlClass myUmlClass : idToClass.values()) {
            if (myUmlClass.isDupRealization()) {
                information.add(myUmlClass.getUmlClass());
            }
        }
        return information;
    }

    public boolean isAllNameExist() {
        return allNameExist;
    }

    public boolean checkInterfaceVisibility() {
        for (MyUmlInterface umlInterface : idToInterface.values()) {
            if (!umlInterface.checkAttributeVisibility()) {
                return false;
            }
        }
        return true;
